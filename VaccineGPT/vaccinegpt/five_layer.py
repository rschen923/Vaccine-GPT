...
class FiveLayerVaccineGPT(nn.Module):
    ...
    def __init__(self, hidden_dim: int = 128, vib_dim: int = 32, domains: int = 8) -> None:
        ...
        self.l1_head = nn.Linear(hidden_dim + 4 + 16, 2)
        ...
        self.l4_head = nn.Linear(hidden_dim + 3, 1)
    ...
    def forward(self, batch: Mapping[str, Tensor]) -> dict[str, Tensor]:
        ...
        h = l0["h_shared"]
        l1_input = torch.cat((h, batch["pcd_A"], batch["ch_covariates"]), dim=-1)
        ...
        l4_input = torch.cat((h, v0.unsqueeze(-1), ecology[:, :2].detach()), dim=-1)
    ...
    def losses(self, batch: Mapping[str, Tensor], output: Mapping[str, Tensor]) -> dict[str, Tensor]:
        ...
        return {
            ...
            "t3": listmle_loss(output["t3"], batch["y3"]),
            ...
            "t4": lambdarank_loss(output["t4"], batch["t4_rel"]),
            "l4_ai": mhc_peptide_loss(output["mhc"], batch["el"], batch["pep_mask"]),
